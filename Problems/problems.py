...
class ProblemSR:
    ...
    def get_problem(self):
        ...
        if self.benchmark_name in ("oes", "llm_srbench"):
            data_root = os.path.join(repo_root, f"{self.benchmark_name}_data")
            if not os.path.isdir(data_root):
                data_root = os.path.join(os.path.dirname(file_path), self.benchmark_name)
        else:
            data_root = os.path.join(os.path.dirname(file_path), self.benchmark_name)

        problem_dir = self.problem_name
        if self.benchmark_name == "llm_srbench" and self.ins_idx is not None:
            problem_dir = f"{self.problem_name}{self.ins_idx}"

        problem_path = os.path.join(data_root, problem_dir)
